...
import "./lockCheck.css"
function LockCheckBox({ setRespLock, id, respLock }) {
  return (
    <div key={id}>
      <input
        id={`inpLock${id}`}
        type="checkbox"
        checked={respLock}
        onChange={() => setRespLock(!respLock)}
        className="inpLock"
      />
      <label className="btn-lock" htmlFor={`inpLock${id}`}>
        <svg width="20" height="24" viewBox="0 0 36 40">
          <path
            className="lockb"
